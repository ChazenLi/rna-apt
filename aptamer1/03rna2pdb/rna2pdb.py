...
# 定义二进制文件和脚本的路径
BIN_PATH = "/home/gxs/Rosetta/main/source/bin"
TOOLS_PATH = "/home/gxs/Rosetta/main/tools/rna_tools"
...
def predict_secondary_structure(fasta_file):
    ...
    result = run_subprocess(["RNAfold", "-i", fasta_file])
    ss = ""
    mfe = 0.0
...
def identify_helices(fasta_file, ss):
    ...
    helix_setup_script = os.path.join(
        TOOLS_PATH, "job_setup/helix_preassemble_setup.py")
    ss_file = fasta_file.replace(".fasta", ".ss")
    with open(ss_file, "w") as f:
        f.write(f">{fasta_file}\n{ss}\n")
    result = run_subprocess(
        ["python2", helix_setup_script, "-secstruct", ss_file, "-fasta", fasta_file])
    helices = []
    for line in result.split('\n'):
...
def compute_structure(fasta_file, ss):
    ...
    command = [de_novo_bin, "-fasta", fasta_file, "-secstruct", ss]
...
def minimize_structure(fasta_file):
    ...
    min_setup_script = os.path.join(
        TOOLS_PATH, "job_setup/parallel_min_setup.py")
...
def select_best_structures(fasta_file):
    ...
    sort_select_script = os.path.join(
        TOOLS_PATH, "silent_util/silent_file_sort_and_select.py")
...
def extract_lowscore_decoys(fasta_file):
    ...
    extract_script = os.path.join(
        TOOLS_PATH, "silent_util/extract_lowscore_decoys.py")
    silent_file = fasta_file.replace(".fasta", ".out")
...
def main():
    rna_sequences = [
        "GCGCCAUGCGUAUAUAUAGCGUCUGAACGCGC",
        # 添加更多RNA序列
    ]
